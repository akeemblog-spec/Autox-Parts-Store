    ...
    <div className="fixed inset-0 z-[420] grid place-items-center overflow-y-auto bg-black/[.78] p-3 backdrop-blur-md sm:p-5" role="dialog" aria-modal="true" aria-labelledby="installment-title" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="relative my-auto w-full max-w-[700px] overflow-hidden rounded-[28px] bg-[#08090b] shadow-[0_30px_100px_rgba(0,0,0,.72),0_0_60px_rgba(237,28,36,.09)] ring-1 ring-inset ring-white/[.1]">
        ...
        <div className="relative p-4 sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-[108px_minmax(0,1fr)] gap-3 sm:grid-cols-[150px_1fr] sm:gap-4">
            <div className="relative grid min-h-[112px] place-items-center overflow-hidden rounded-2xl bg-[radial-gradient(circle_at_center,rgba(237,28,36,.2),transparent_58%),#050506] ring-1 ring-inset ring-autox-red/25 sm:min-h-[150px]">
              {product.image ? <img src={product.image} alt={product.name} className="h-[96px] w-[96px] object-contain drop-shadow-[0_14px_28px_rgba(237,28,36,.28)] sm:h-[138px] sm:w-[138px]"/> : <CreditCard size={42} className="text-autox-red"/>}
            </div>
            <div className="flex min-w-0 flex-col justify-center rounded-2xl bg-white/[.035] p-3 ring-1 ring-inset ring-white/[.07] sm:p-4">
          ...
          <div className="mt-4 rounded-2xl bg-[#0d0f12] p-4 ring-1 ring-inset ring-white/[.08]">
            <div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-autox-red/10 text-autox-red ring-1 ring-inset ring-autox-red/20"><CreditCard size={18}/></span><div><p className="text-sm font-extrabold text-white">Installment Option</p><p className="text-[10px] text-zinc-500">KOKO / Mintpay where enabled at checkout</p></div></div><span className="inline-flex shrink-0 items-center justify-center whitespace-nowrap rounded-full bg-black/40 px-3 py-1.5 text-center text-[9px] font-black uppercase leading-none tracking-wide text-autox-red ring-1 ring-inset ring-autox-red/30 sm:justify-start sm:text-left">3 or 6 months</span></div>
          </div>
