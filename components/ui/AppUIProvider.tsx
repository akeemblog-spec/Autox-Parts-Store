...
export function AppUIProvider({ children }: { children: React.ReactNode }) {
  ...
  return <AppUIContext.Provider value={value}>
    ...
    {confirmState && <div className="fixed inset-0 z-[200] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-autox-border bg-[#0c0c0c] shadow-2xl">
        ...
        <div className="flex justify-end gap-3 p-4"><button onClick={() => closeConfirm(false)} className="rounded-xl border border-autox-border px-4 py-2 text-sm font-semibold text-autox-gray hover:text-white">{confirmState.cancelLabel ?? "Cancel"}</button><button onClick={() => closeConfirm(true)} className={`rounded-xl px-4 py-2 text-sm font-bold text-white ${confirmState.destructive ? "bg-autox-red hover:bg-autox-redDark" : "bg-white/10 hover:bg-white/15"}`}>{confirmState.confirmLabel ?? "Confirm"}</button></div>
      </div>
    </div>}
