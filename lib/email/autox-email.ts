...
export async function sendTransactionalEmail({ to, subject, html }: { to: string; subject: string; html: string }) {
  ...
  const from = process.env.EMAIL_FROM || "AutoX Parts Store <onboarding@resend.dev>";
  if (!key) {
    if (process.env.NODE_ENV !== "production") console.warn(`[AutoX email preview] ${subject} -> ${to}`);
